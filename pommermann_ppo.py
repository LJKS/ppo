...
class Network:
    def __init__(self, input_size, layers, outputs, name, graph):
        """
        Args:
        ...
        for i,layer in enumerate(layers):
            ...
            self.layer_list.append(self._fully_connected_layer(layer_input, layer_input_size, layer['size'], layer['activation'], layer_name, graph))

        #Create outputs
        for i, output in enumerate(outputs):
            output_name = name+'_output_'+str(i)
            self.output_list.append(self._fully_connected_layer(self.layer_list[-1], layers[-1]['size'], output['size'], output['activation'], output_name, graph))

        with self.graph.as_default():
            self.saver = tf.train.Saver(self.save_list, max_to_keep=999999)



    def _fully_connected_layer(self, input, input_size, size, activation, name, graph):
        ...
        with graph.as_default():
            ...
            bias = tf.get_variable(bias_name, shape=size, initializer=tf.glorot_uniform_initializer)
            weights = tf.get_variable(weights_name, shape=[input_size, size], initializer=tf.glorot_uniform_initializer)
    ...
    def save(self, session, file_name, iteration):
        with self.graph.as_default():
            print(self.saver.save(sess=session, save_path=file_name, global_step=iteration))

    def restore(self, session, file_name, iteration):
        with self.graph.as_default():
            restore_name = file_name + '-' + str(iteration)
            self.saver.restore(session, restore_name)

class PPO_model:
    def __init__(self, actor_description, critic_description, env_name, file_name=None, load_iteration=None, graph=None):
        """
        Args:
        ...
        self.iteration = 0 if load_iteration==None else load_iteration
        #String used by gym to create environment
        self.env_name = env_name
        #filename to use to save and to load - its the path rather than an acutal file (see tf.Saver().save())
        ...
        self.actor = Network(self.input_size, actor_description, [{'size': self.output_size,'activation':ACTION_ACTIVATION}], 'actor', self.graph)
        self.critic = Network(self.input_size, critic_description, [{'size':1, 'activation':None}], 'critic', self.graph)
    ...
    def sample_action_from_network(self, observation):
        with self.graph.as_default():
            feed_dict = self.feed_dictionary(observation)
            action, _, _ = self.session.run(fetch_list, feed_dict=feed_dict)
            return action
    ...
    def create_data(self):
        ...
        action_fetch, log_prob_fetch = self.retrieve_action()
    ...
    def train_step(self):
        ...
        with self.graph.as_default():
            policy_loss = self.actor_optimizer.minimize(self.policy_loss())
            critic_loss = self.critic_optimizer.minimize(self.value_loss())
    ...
    def save(self):
        self.critic.save(self.session, self.file_name + '_critic', self.iteration)
        self.actor.save(self.session, self.file_name + '_critic', self.iteration)
...
def main():
    ...
    trainer = PPO_model(network_description, network_description, 'LunarLander-v2', file_name='saves/LunarLander-v2', load_iteration=10)
    evaluations = []
